<template>
    <div>
        <form method="get" @submit="onSubmit">
            <div class="form-group">
                <label for="fileQuery">Enter name of the file to load from server:</label>
                <input class="form-control" type="text" placeholder="filename.txt" id="fileQuery" v-model="filename">
            </div>
    ...
    export default {
        ...
        methods: {
            onSubmit(event) {
                ...
                console.log("onSubmit", this.filename)
                this.filename = this.filename.trim()
                if (this.filename) {
                    fetch(this.$hostname + "/server/file?name=" + this.filename)
                        .then(data => data.json())
                        .then(json => {
                          if (json['status'] === 'error') {
                           this.$emit('reload')
                          }
                              this.$emit('encodeText', this.filename, json['content'][0], json['content'][1])
                        })
